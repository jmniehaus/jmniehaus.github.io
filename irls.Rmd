...
**Note:** *The material presented here is a consolidation and re-statement of work that has already been done. See supporting references.*

**Note also:** *The programming in R follows the math as presented; however, it would* never *be a good idea to implement IRLS in this way, as the computation presented here is very inefficient (e.g., taking the inverse of a matrix without a factorization).*
...
For a bit of background, IRLS is an iterative method that can (among other things) be used to find the coefficients of a generalized linear model.[^1] At each iteration, it utilizes weighted least squares until convergence to the vector of maximum likelihood estimates (MLEs). The method relies on the Newton-Raphson algorithm, but is equivalent to Fisher scoring when using the canonical link (e.g., logit for logistic regression, and log for Poisson).[^2] However, at each iteration of the Newton-Raphson algorithm, IRLS computes the closed-form solution to a weighted least squares problem. Interestingly, then, it takes an optimization problem without a closed-form solution, and turns it into one that does have a solution at each iteration. Unfortunately, the Newton-Raphson algorithm requires an analytic first and second derivative (or good approximations) in order to work, so often times alternative, quasi-Newton methods are used for numerical optimization. These methods do not require analytic derivatives to converge, and can therefore be used when analytic derivatives are not easily found. 

Because IRLS relies on the Newton-Raphson algorithm to converge, it follows that first and second partials are necessary. Although this can be expensive, computing the gradient vector and Hessian matrix is fairly simple for typical GLMs, and therefore lends to IRLS quite nicely.  By supplying an analytic first and second derivative, we forgo having to approximate these functions, and therefore converge fairly fast to the vector of MLEs. Alternative, quasi-Newton methods must approximate both the first and second derivatives by a differencing approach, which can be computationally costly. 

\

### Why study it?

My initial motivation to study IRLS came about when I was using R's `optim()` command to find the maximum of a custom-defined likelihood. In studying the source-code of the `glm()` function, I found that it was not using quasi-Newton methods (as is mostly the case with `optim`), which lead me to study IRLS. Thus, aside from some of the clear efficiency and accuracy gains from IRLS over other numerical optimization procedures for estimating GLMs, there is another, more intrinsically valuable reason to study IRLS. Namely, R's `glm` command is in fact running the IRLS algorithm "under the hood," so to speak (and most canned GLM packages do so, too). That is, when user's specify a GLM such as `glm(y~x, family=binomial)` in R, the underlying routine (`glm.fit`, for those interested) makes a call to C, which then makes a call to FORTRAN, with FORTRAN finally running the IRLS algorithm ([This link](https://madrury.github.io/jekyll/update/statistics/2016/07/20/lm-in-R.html) has a deep exposition on the process, for those interested). Thus, despite how many social scientists may be taught MLE and GLMs (i.e., quasi-Newton methods), the actual procedure utilized by R is in fact IRLS.
...
and we want to approximate the location at which it crosses the x-axis. That is, we want to find the zero, or root of the function without having to use the quadratic equation or some other analytic method (indeed, we often *cannot* find a closed-form solution in the case of GLMs). We can turn to Newton's method to do so. 

In addition to the above function, suppose that we have a single point $(x_0,f(x_0))$ available to us. The point $x_0$ could simply be a guess about the root, with the corresponding function evaluation $y_0=f(x_0)$ there. Because we are given the function in this scenario, we then know from calculus that the slope of the function at this point is $f'(x_0)$. So we have a point on the function, $y_0 = f(x_0)$, and the slope at that point $f'(x_0)$. Further, notice that the point $(x_0,f(x_0))$ on the function is, by definition, also a point on the tangent line. Because we have a point on the tangent, as well as that tangent's slope, we can then linearly approximate the function using the point-slope form of the tangent at $(x_0,f(x_0))$:
...
Alternatively, we could look at this as a Taylor series expansion of $f(x)$ about the point $x_0$, where we simply drop the higher order polynomials altogether. The Taylor Series view will become important when applying the above to ML estimation. 

Graphically, suppose we select an arbitrary point $x_0$, and draw its corresponding linear approximation to $f(x)$ as in the process just described:
...
newt.plot = function(x0, fn, fprime, from, to,
  ...
  labs = as.expression( sapply(labseq, function(x) bquote("x"[.(x)])))
...
Glancing at the plot, it looks like the point at which the red tangent intersects the x-axis is somewhat closer to the function's zero than is the initial guess, $x_0$. So, what if we updated our guess about where the zero is, from $x_0$ to $x_1$, where $x_1$ is the point where the original red tangent intersects the x-axis? Lets find out. 
...
Now that we have $x_1$, we can obtain the corresponding tangent/linear approximation to $f(x)$, but this time at $f(x_1)$. Recall, $x_1$ is the point at which $x_0$'s tangent intersected the x-axis. Graphically, then: 
...
\begin{align}
\L(\bm{\beta}| \boldsymbol{X}, \bm{y}) &= \prod_{i=1}^n f(y_i | \bm x_i) \nonumber \\
\implies \ln\L(\bm{\beta}| \boldsymbol{X}, \bm{y}) &= \sum_{i=1}^n \ln f(y_i|\bm{x}_i) \label{like}
\end{align}

where:

* $f(\cdot)$ is the marginal probability distribution function
...
Just as we wanted to find the zeros of a function in the preceding review of Newton's Method, we want to do so as well in equation \\ref{foc}, but now we are finding the zeros of the first derivative instead of the original function. That is, we want to find the values of $\beta=\hat\beta$ where the first derivative function equals zero. This location will be a local extremum of the likelihood, and can be arrived at by Newton's method. 

Specifically, we take the gradient of the log-likelihood with respect to $\beta$. Since we want to find the zeros of this gradient, we take its Taylor series expansion, again ignoring higher order polynomials. Expanding the gradient of the log-likelihood about the point $\bm b_0$, with the gradient denoted $\nabla(\bm{\beta})$, we arrive at a linear approximation to the gradient of the log-likelihood with respect to the parameter vector:

\begin{align*}
\nabla(\bm{\beta}) = \nabla(\bm b_0) + \bm{\mathcal{H}}(\bm b_0)(\bm \beta - \bm b_0)
\end{align*}
...
* $\nabla(\bm b_0)$ is the $k \times 1$ gradient vector of the log-likelihood evaluated at $b_0$
...
* $\bm \gamma$ is an $n \times 1$ vector, with elements $\gamma_i = f_\mu$
...
1. The first partial of the log-likelihood $\equiv f_{\mu_i}$ with respect to $\mu_i = \bm x_i^\top \bm\beta$ 

2. The second partial of the log-likelihood $\equiv f_{\mu_i\mu_i}$ with respect to $\mu_i = \bm x_i^\top \bm\beta$ .

With each of these equations in hand, the chain rule tells us that we can simply multiply through by $\bm X^\top$ and $\bm X^\top(\cdot)\bm X$ to arrive at the the corresponding $\nabla(\bm \beta)$ and $\bm{\mathcal{H}}(\bm\beta)$ that we need in order to recast Newton's method in terms of weighted least squares. As such, for each of the methods considered below, both of these quantities are derived.
...
```{r, echo=T, eval=T}
`%!in%` = Negate(`%in%`)
irls = function(fu, fuu, X, y, method, starts=NULL, itmax=100, tol = 1e-3){
  if(method %!in% c("logistic","poisson")) stop("`method` must be one of either `logistic`, or `poisson`")
...
And just for the sake of clarity, the empirical covariance structure does approximate the desired, theoretical structure:

```{r, echo=T}
list(theoretical_cov=Sigma, empirical_cov=cov(X[,-1]))
```
...
```{r, echo=T, eval=T}
irls(fu_logistic, fuu_logistic, X, y, itmax=50, method="logistic")
...
Now that we have the general `irls` function defined, all we have to do in order to estimate the Poisson parameters is define the derivative functions that have just been derived, and then generate some data from a Poisson distribution.
