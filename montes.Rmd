...
Recall that the Weak Law of Large Numbers (WLLN) states$$ \lim_{n\to\infty} \frac{1}{n}\sum_{i=1}^n g(X_i) \longrightarrow \E[g(X)] = \int_{X} g(x)f_X(x) \mathrm dx$$ for random variables $X_i \overset{iid}{\sim} f_X, \,\, i\in 1,2,...,n$, and (almost) any function $g$. This effectively says that the sample average of nearly any function of the data converges in probability to the true expected value. For example $$ \lim_{n\to\infty} \frac{1}{n}\sum_{i=1}^n X_i = \mu_x $$ meaning that the sample mean converges to the population mean as the sample size tends to infinity, where $g$ in this case was the identity function, $g(X_i) = X_i$. (*Note*: although the definition here assumes a continuous density $f$, the same law holds for discrete densities and can be denoted using either summation or a Riemann-Stieltjes integral when taking the expectation.) 

Despite being extremely simple, the power of the WLLN as it relates to Monte Carlo analysis cannot be overstated. This lies in the fact that nearly everything we are interested in analyzing about a random variable (e.g., estimators, in particular) is a function $g$ of a random variable, and is therefore subject to the WLLN. A few examples (in addition to the identity function above): 

* The variance: $$g(X_i) = \frac{1}{n-1} \sum_{i=1}^n (X_i - \bar X)^2 \longrightarrow \mathrm{var}(X)$$

* The CDF: $$g(X_i) = \frac{1}{n} \sum_{i=1}^n \mathbb{I}_{(-\infty, x)}(X_i) \longrightarrow F_X(x)$$

* The density function: $$g(X_i) = \text{The empirical density/mass function} \longrightarrow f_X(x)$$

* The quantile function: $$g(X_i) = \text{The empirical } \alpha\text{-quantile of } X_i, ..., X_n \longrightarrow \text{inf}(y: F(x) \geq \alpha)$$

So effectively we can simulate random variables, compute some function of those realizations, and then take the mean of the function, and the WLLN tells us that we converge to the expected value of that function. For example, we can simulate data, compute least squares coefficients in a linear regression for that data, and then compute the mean/variance/CDF/PDF/quantiles of these coefficients. These computations converge in probability to their true values. With these quantities in hand, we can then assess things like the bias of estimators, the variance of estimators, their power and size, type II error, etc, *all just because the sample average converges to the true average*. 
