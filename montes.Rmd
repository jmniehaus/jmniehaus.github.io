...
```{r, echo=F, eval=F}
# Install, load necessary packages
pks = c("RhpcBLASctl", "doParallel", "doRNG", "MASS")
needed = setdiff(pks, rownames(installed.packages()))
if(length(needed) > 0)
  install.packages("pks", Ncpus = parallel::detectCores() - 1)
...
simulation_fn = function(grid_row) {
  ...
  Z = cbind(
    X,
    rbeta(n, 18, 7) # Exclusion restriction
  ) 
  
  # simulate correlated errors
  Sigma = matrix(grid_row[["sig_eu"]], ncol=2, nrow=2)
  diag(Sigma) = 1
  
  errs = MASS::mvrnorm(n, mu = c(0,0), Sigma = Sigma)
  colnames(errs) = c("u", "e")
  ...
  # observed indicator
  obs = ifelse(eta_select + errs[, "u"] > 0 , 1, 0)
  
  # measurement eqn
  y = eta_meas + errs[, "e"]
  y[obs == 0] = NA
  
  out = cbind.data.frame(X = X, Z = Z, y = y, obs = obs)
  return(out)
...
estimation_fn = function(dat, grid_row, iter){
  ...
  mod = lm(y ~ X.2 + X.3 + imills, 
           data = dat, 
           subset = obs == 1)
  
  prop_obs = sum(dat$obs) / nrow(dat)
  
  out = summary(mod)$coefficients[1:3, 1:2]
  
  colnames(out) = c("est", "se")
  rownames(out) = c("b1", "b2", "b3")
...
n = c(50, 500, 5000)
sig_eu = c(.05, .25, .5, .75, .95)

nsims = 500
grid = expand.grid(n = n, sig_eu = sig_eu)
grid_multi = do.call(rbind.data.frame, replicate(nsims, grid, simplify = F))
