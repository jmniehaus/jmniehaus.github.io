...
```{r, echo=F, eval=T}
setup_cl = function(nframes=1){

  eval.parent(
    expression({
    
      old_blas = blas_get_num_procs()
      old_omp = omp_get_num_procs()
      blas_set_num_threads(1)
      omp_set_num_threads(1)
...
sums = res %>% 
  group_by(n) %>% 
  summarize(size = mean(abs(est/se) > qt(.95, n - 2)),
            r2_avg = mean(r2)) %>%
  pivot_longer(cols = c(size, r2_avg))

ggplot(data=sums, aes(x = factor(n), y = value, fill=name)) + geom_col(position="dodge", alpha=.75)
