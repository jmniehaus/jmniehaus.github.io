...
**Note** that this guide will applies mostly to the case of trivial parallelism, i.e., simulations that are independent of each other. However some concepts can be extended to dependent simulations, particularly in MCMC algorithms that rely on full-conditionals. Where this is the case, I note it in the Caveats subsection of a topic. 
...
* The variance: $$\frac{1}{n-1} \sum_{i=1}^n (X_i - \bar X)^2 \longrightarrow \mathrm{var}(X)$$

* The CDF: $$\frac{1}{n} \sum_{i=1}^n \mathbb{I}_{(-\infty, x)}(X_i) \longrightarrow F_X(x)$$

* The density function: $$\text{The empirical density/mass function} \longrightarrow f_X(x)$$

* The quantile function: $$\text{The empirical } \alpha\text{-quantile of } X_i, ..., X_n \longrightarrow \text{inf}(y: F(x) \geq \alpha)$$
...
To show that this is indeed true, we will need a a few pieces of background information, namely Markov's Inequality, Chebyshev's inequality, and the definition of convergence in probability (note that all of this information can also be found in most math-stat textbooks, or on Wikipedia).
...
\begin{equation*} \lim_{n\to\infty} \frac{1}{n}\sum_{i=1}^n g(X_i) \overset{p}\longrightarrow \E[g(X_n)] = \int_{X} g(x)f_X(x) \mathrm dx \end{equation*} 
...
As implied by the review of Monte Carlo analysis, simulations will involve a great number of iterations in order to obtain a sufficiently large sample to be averaged over, such that a law of large numbers can take adequate effect. In many cases, implementing these Monte Carlo analyses in parallel will be the first step to obtaining results efficiently. There are exceptions to this rule (e.g., reserving computing resources for other tasks, parallel overhead making things slower), but if the reason you are not using parallel processing is simply because you do not know how, then I'll walk you through a few approaches here. 
