...
```{r, echo=F, eval=T}
setup_cl = function(ncpus = (parallel::detectCores() - 1),
                    nframes = 1 ){
...
inst_load = function(pks = NULL,
  ...
  pks =  c(pks, 'RhpcBLASctl', 'doParallel', 'doRNG')
...
sim_and_est = function(n, iter_id){
  ...
  out = c(iter_id = iter_id, 
          n = n,
          sumr$coefficients[2, 1:2], 
...
colnames(res)= c("iter_id", "n", "est", "se", "r2")
...
ggplot(data = sums, aes(x = factor(name), y = value, fill = factor(n))) + 
  ...
  labs(title = expression(paste("Type I Error Rates (", alpha==0.05,") and R-Squared in Spurious Regression")),
       x = "Quantity",
       y = "Value"
       ) +
...
estimation_fn = function(dat, grid_row, iter_id){
  ...
  return(
    ...
    c(iter_id = iter_id, coef(mod), grid_row)
...
sums = res %>% 
  dplyr::group_by(n, p) %>% 
  dplyr::summarize(mean_lag = mean(lag_y)) %>% 
...
```{r}
res = as.data.frame(do.call(rbind, out))
res_sub = res[res$param == "b2",]
res_sub[,c("n", "b1", "prop_obs", "est", "se")] = lapply(res_sub[,c("n", "b1", "prop_obs", "est", "se")], as.numeric)

sums = res_sub %>% 
  dplyr::group_by(n, b1, model) %>%
  dplyr::summarize(avg_b = mean(est),
                   avg_obs = mean(prop_obs)) %>%
  dplyr::mutate(bias = 100*(-avg_b/1.2 - 1))
  
ggplot(sums, aes(x=model, y=bias, fill=factor(round(avg_obs, 2)))) + 
  geom_col(position="dodge", alpha = .6) +
  facet_wrap(vars(n)) +
  scale_fill_viridis_d(begin = .25, 
                       end = .75, 
                       name = "Proportion Observed"
                       ) 
